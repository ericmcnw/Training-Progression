...
import CompletionLogForm from "./CompletionLogForm";

export const dynamic = "force-dynamic";

type Params = { id: string };

export default async function LogCompletionPage(props: { params: Promise<Params> | Params }) {
  ...
  return (
    <div style={{ maxWidth: 980, margin: "0 auto", padding: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 900, margin: 0 }}>{routine.name} - Log Completion</h1>
      ...
      <section style={panel}>
        <div style={panelHeader}>COMPLETION DETAILS</div>
      ...
      <section style={{ ...panel, marginTop: 16 }}>
        <div style={panelHeader}>RECENT COMPLETION LOGS</div>
        <div style={{ padding: 12, display: "grid", gap: 8 }}>
          {recentLogs.length === 0 && <div style={{ opacity: 0.75 }}>No completion logs yet.</div>}
          {recentLogs.map((log) => (
            <div key={log.id} style={card}>
              <div style={{ fontWeight: 800 }}>{new Date(log.performedAt).toLocaleString()}</div>
              {log.completionCount ? <div style={{ opacity: 0.8, marginTop: 2 }}>Count: {log.completionCount}</div> : null}
