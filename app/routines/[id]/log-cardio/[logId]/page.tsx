...
import EditRunLogForm from "./EditRunLogForm";
...
export default async function EditCardioLogPage(props: {
  ...
  const defaultReturnTo = `/routines/${routineId}/log-cardio`;
  ...
  const log = await prisma.routineLog.findUnique({
    where: { id: logId },
    select: { id: true, routineId: true, performedAt: true, distanceMi: true, durationSec: true, notes: true },
  });
  if (!log || log.routineId !== routineId) return <div style={{ padding: 20 }}>Log not found for this routine.</div>;

  return (
    <div style={{ maxWidth: 980, margin: "0 auto", padding: 20 }}>
      <h1 style={{ fontSize: 24, fontWeight: 900, margin: 0 }}>{routine.name} - Edit Cardio Log</h1>
